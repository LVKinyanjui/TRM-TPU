...
def convert_subset(set_name: str, config: DataProcessConfig):
    ...
    print(f"[{set_name}] Downloading CSV from Hugging Face...")
    ...
    print(f"[{set_name}] Augmenting data...")
    ...
    def _seq_to_numpy(seq):
        # TPU CHANGE: Forced int32 conversion for XLA compatibility
        arr = np.concatenate(seq).reshape(len(seq), -1).astype(np.int32)
        assert np.all((arr >= 0) & (arr <= 9))
        return arr + 1
    
    print(f"[{set_name}] Finalizing Numpy tensors...")
    results = {
